...
| 매일 아침 | 오늘의 일정 전체를 한 메시지로 묶어서 |

리마인더 시간은 설정으로, 아침 알림 시각은 cron 등록 시각으로 정합니다.

아침 알림은 오늘에 걸쳐 있는 일정을 종류를 가리지 않고 모두 담습니다. 하루를
시작할 때 보는 요약이므로 리마인더와 겹치는 것은 의도된 동작입니다. 같은 일정이
아침에 한 번, 시작 직전에 한 번 보입니다.

날짜는 헤더에 한 번만 나오고 각 줄은 시각만 보여줍니다 (`오후 2:00-오후 3:00`).
여러 날에 걸친 일정은 `어제-내일`처럼 오늘 기준 상대 표기로, 하루를 넘게 벗어나면
`7월 30일-8월 8일`처럼 날짜로 나옵니다. 오늘 하루짜리 하루종일 일정은 `하루종일`
입니다. 시각이 있는 일정이 시간순으로 먼저 오고 하루종일 일정이 뒤에 붙습니다.
여러 날에 걸친 일정은 진행 중인 동안 매일 표시됩니다.
...
## 아침 알림이 DB 대신 구글에 직접 묻는 이유

증분 동기화(`syncToken`)는 반복일정을 **마스터 이벤트 하나로만** 돌려줍니다.
회차별로 주지 않습니다. 그래서 주간 회의는 시리즈가 시작된 날짜에 딱 한 번
나타나고 그 뒤로는 영영 나오지 않습니다. DB만으로는 오늘이 몇 번째 회차인지 알 수
없습니다.

`--daily_digest` 실행은 DB를 아예 열지 않고 구글에 `singleEvents=True`로 오늘
하루치를 직접 질의합니다. 이 옵션이 시리즈를 회차로 펼쳐서 돌려줍니다. sync token은
읽지도 쓰지도 않으므로 기존 동기화 루프를 건드릴 수 없습니다.
...
이 설정은 시각 표시 형식과, 아침 알림에서 어느 날이 "오늘"인지를 결정합니다.
...
매분 실행이 변경 감지와 리마인더를 담당합니다. 아침 실행은 오늘의 일정만 보내고
DB를 열지 않은 채 종료하므로, 두 실행이 같은 초에 시작해도 안전합니다.
...
--daily_digest            오늘의 일정을 한 메시지로 보내고 종료
