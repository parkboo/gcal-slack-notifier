...
> 이 한국어 문서가 정본입니다. 내용이 어긋나면 한국어판이 맞습니다.

## 이런 팀에 맞습니다

구글 캘린더를 일정 관리 도구로 사용하면서 서버를 직접 운영하고 구글 서비스 계정을 만들 수 있는 팀. 
원클릭 슬랙 앱이 아니라 cron 스크립트입니다. *Add to Slack* 버튼은 없고, 캘린더마다 서비스 계정에 공유
...
슬랙은 이 앱을 후속으로 안내하고 있지만, 공유 팀 일정이 추가·변경·취소될 때 원하는 채널로 알려주지는 않습니다.
...
| 상황 | 메시지 |
|---|---|
| 일정 추가 | 제목과 날짜, 일정으로 가는 링크 (장소가 있으면 함께) |
| 일정 제목·날짜 변경 | 변경 전 → 변경 후 |
| 일정 취소 | 제목과 날짜 |
| 일정 시작 15분 전 | 현지 시각이 표시된 리마인더 |
| 매일 아침 | 오늘의 하루종일 일정과 반복일정 회차를 한 메시지로 묶어서 |

리마인더 시간은 설정으로, 아침 알림 시각은 cron 등록 시각으로 정합니다.

아침 알림에 하루종일 일정과 반복일정만 담는 이유는, 이 둘만 다른 경로로 알림이
가지 않기 때문입니다. 하루종일 일정은 시작 시각이 없어 리마인더에 걸리지 않고,
반복일정 회차는 DB에 마스터만 저장되므로 리마인더 대상이 아닙니다. 시간이 지정된
단발 일정은 이미 시작 전 리마인더가 가므로 넣지 않습니다. 여러 날에 걸친 일정은
진행 중인 동안 매일 표시됩니다.

일정에 장소가 있으면 📍와 함께 표시됩니다. 장소에 화상회의 링크를 적어둔 경우
채널에는 그대로 나오지만 푸시 알림에서는 URL을 걷어냅니다. 장소만 바뀐 경우에는
알림을 보내지 않습니다. 회의실 이름에 참석자를 함께 적어두면 인원이 바뀔 때마다
알림이 쏟아지기 때문입니다.
...
이 설정은 시각 표시 형식과, 다이제스트에서 어느 날이 "오늘"인지를 결정합니다.
...
cron 두 줄이 컨테이너 안에서 돕니다. 아침 알림 시각을 바꾸려면 `docker/crontab`의
`0 9`를 원하는 시각으로 수정하세요.
